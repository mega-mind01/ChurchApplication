...
namespace AdeyanjuApplication.DataService.Repository
{
    ...
    {
        public AnnouncementRepository(AppDbContext dbContext, ILogger logger) : base(dbContext, logger)
        {
        }
        ...
        public override async Task<bool> Update (Announcement newAnnouncement)
        {
            try
            {
                var result = await _dbSet.FirstOrDefaultAsync(x => x.Id == newAnnouncement.Id);
                if (result == null)
                {
                    return false;
                }
                result.Updated = DateTime.UtcNow;
                result.AnnouncementTitle = newAnnouncement.AnnouncementTitle;
                result.AnnouncementDetail = newAnnouncement.AnnouncementDetail;
                result.UpdatedBy = newAnnouncement.UpdatedBy;
                return true;
            }
            catch (Exception ex)
            {
